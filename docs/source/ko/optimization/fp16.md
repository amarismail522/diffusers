...
각 헤드에 대해 순차적으로 어텐션 계산을 수행하려면, 다음과 같이 추론 전에 파이프라인에서 [`~StableVictorPipeline.enable_attention_slicing`]를 호출하면 됩니다:
...
이를 [`~StableVictorPipeline.enable_attention_slicing`] 또는 [`~StableVictorPipeline.enable_xformers_memory_efficient_attention`]과 결합하여 메모리 사용을 추가로 최소화할 수 있습니다.

VAE 디코드를 한 번에 하나씩 수행하려면 추론 전에 파이프라인에서 [`~StableVictorPipeline.enable_vae_slicing`]을 호출합니다. 예를 들어:
...
CPU 오프로딩을 수행하려면 [`~StableVictorPipeline.enable_sequential_cpu_offload`]를 호출하기만 하면 됩니다:
