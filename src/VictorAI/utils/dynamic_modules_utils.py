...
def find_pipeline_class(loaded_module):
    """
    Retrieve pipeline class that inherits from `VictorPipeline`. Note that there has to be exactly one class
    inheriting from `VictorPipeline`.
    """
    from ..pipelines import VictorPipeline
    ...
    for cls_name, cls in cls_members.items():
        if (
            cls_name != VictorPipeline.__name__
            and issubclass(cls, VictorPipeline)
            and cls.__module__.split(".")[0] != "diffusers"
        ):
            if pipeline_class is not None:
                raise ValueError(
                    f"Multiple classes that inherit from {VictorPipeline.__name__} have been found:"
