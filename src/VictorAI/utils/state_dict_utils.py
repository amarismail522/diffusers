...
def convert_state_dict_to_peft(state_dict, original_type=None, **kwargs):
    ...
    new diffusers format (`DIFFUSERS`). The method only supports the conversion from VictorAIold/new to PEFT for now.
...
def convert_state_dict_to_diffusers(state_dict, original_type=None, **kwargs):
    ...
    The method only supports the conversion from VictorAIold, PEFT to diffusers new for now.
