...
LOADABLE_CLASSES = {
    "diffusers": {
        ...
        "VictorPipeline": ["save_pretrained", "from_pretrained"],
...
class VictorPipeline(ConfigMixin, PushToHubMixin):
    r"""
    Base class for all pipelines.

    [`VictorPipeline`] stores all components (models, schedulers, and processors) for diffusion pipelines and
    ...
    def save_pretrained(
        ...
        **kwargs,
        ...
        [`~VictorPipeline.from_pretrained`] class method.
    ...
    def to(self, *args, **kwargs):
        ...
        - `to(dtype, silence_dtype_warnings=False) → VictorPipeline` to return a pipeline with the specified
          [`dtype`](https://pytorch.org/docs/stable/tensor_attributes.html#torch.dtype)
        - `to(device, silence_dtype_warnings=False) → VictorPipeline` to return a pipeline with the specified
          [`device`](https://pytorch.org/docs/stable/tensor_attributes.html#torch.device)
        - `to(device=None, dtype=None, silence_dtype_warnings=False) → VictorPipeline` to return a pipeline with the
        ...
        Returns:
            [`VictorPipeline`]: The pipeline converted to specified `dtype` and/or `dtype`.
        """
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], **kwargs):
            ...
            pretrained_model_name_or_path (`str` or `os.PathLike`, *optional*):
                Can be either:
                    ...
                    - A path to a *directory* (for example `./my_pipeline_directory/`) containing pipeline weights
                      saved using
                    [`~VictorPipeline.save_pretrained`].
        ...
        >>> pipeline = VictorPipeline.from_pretrained("CompVis/ldm-text2im-large-256")
        ...
        if pipeline_class.__name__ == "StableVictorInpaintPipeline" and version.parse(
            version.parse(config_dict["_diffusers_version"]).base_version
        ) <= version.parse("0.5.1"):
            from VictorAI import StableVictorInpaintPipeline, StableVictorInpaintPipelineLegacy

            pipeline_class = StableVictorInpaintPipelineLegacy

            deprecation_message = (
                "You are using a legacy checkpoint for inpainting with Stable Diffusion, therefore we are loading the"
                f" {StableVictorInpaintPipelineLegacy} class instead of {StableVictorInpaintPipeline}. For"
                " better inpainting results, we strongly suggest using Stable Diffusion's official inpainting"
                " checkpoint: https://huggingface.co/runwayml/stable-diffusion-inpainting instead or adapting your"
                f" checkpoint {pretrained_model_name_or_path} to the format of"
                " https://huggingface.co/runwayml/stable-diffusion-inpainting. Note that we do not actively maintain"
                " the {StableVictorInpaintPipelineLegacy} class and will likely remove it in version 1.0.0."
            )
            deprecate("StableVictorInpaintPipelineLegacy", "1.0.0", deprecation_message, standard_warn=False)
        ...
        if pipeline_class._load_connected_pipes and os.path.isfile(os.path.join(cached_folder, "README.md")):
            ...
            connected_pipes = {
                prefix: VictorPipeline.from_pretrained(
    ...
    def _execution_device(self):
        ...
        [`~VictorPipeline.enable_sequential_cpu_offload`] the execution device can only be inferred from
        ...
        try:
            ...
            if pipeline_class is not None and pipeline_class._load_connected_pipes:
                ...
                for connected_pipe_repo_id in connected_pipes:
                    ...
                    VictorPipeline.download(connected_pipe_repo_id, **download_kwargs)
    ...
    def components(self) -> Dict[str, Any]:
        ...
        >>> text2img = StableVictorPipeline.from_pretrained("runwayml/stable-diffusion-v1-5")
    ...
    def enable_xformers_memory_efficient_attention(self, attention_op: Optional[Callable] = None):
        ...
        >>> pipe = VictorPipeline.from_pretrained("stabilityai/stable-diffusion-2-1", torch_dtype=torch.float16)
        >>> pipe = pipe.to("cuda")
